...
describe('DoubleRatchet', () => {
    ...
    it('encrypts and decrypts a single message (Alice → Bob)', () => {
        ...
        const { ciphertext, nonce, ratchetPublicKey, previousChainLength, messageNumber, nextState: aliceNext, } = DoubleRatchet.encrypt(aliceState, plaintext);
        expect(messageNumber).toBe(0);
        expect(aliceNext.sendMessageNumber).toBe(1);
        const { plaintext: decrypted, nextState: bobNext } = DoubleRatchet.decrypt(bobState, ratchetPublicKey, previousChainLength, messageNumber, nonce, ciphertext);
    ...
    it('handles multiple messages in one direction', () => {
        ...
        for (let i = 0; i < 5; i++) {
            const { plaintext, nextState } = DoubleRatchet.decrypt(bobCurrent, messages[i].ratchetPublicKey, messages[i].previousChainLength, messages[i].messageNumber, messages[i].nonce, messages[i].ciphertext);
    ...
    it('handles bidirectional communication with ratchet steps', () => {
        ...
        const dec1 = DoubleRatchet.decrypt(bobState, enc1.ratchetPublicKey, enc1.previousChainLength, enc1.messageNumber, enc1.nonce, enc1.ciphertext);
        ...
        const dec2 = DoubleRatchet.decrypt(aliceState, enc2.ratchetPublicKey, enc2.previousChainLength, enc2.messageNumber, enc2.nonce, enc2.ciphertext);
        ...
        const dec3 = DoubleRatchet.decrypt(bobState, enc3.ratchetPublicKey, enc3.previousChainLength, enc3.messageNumber, enc3.nonce, enc3.ciphertext);
    ...
    it('handles out-of-order messages', () => {
        ...
        const msgs = [];
        for (let i = 0; i < 3; i++) {
            const result = DoubleRatchet.encrypt(aliceState, new TextEncoder().encode(`msg-${i}`));
            msgs.push(result);
            aliceState = result.nextState;
        }
        // Bob receives msg 2 first (skipping 0 and 1)
        const { plaintext: p2, nextState: s1 } = DoubleRatchet.decrypt(bobState, msgs[2].ratchetPublicKey, msgs[2].previousChainLength, msgs[2].messageNumber, msgs[2].nonce, msgs[2].ciphertext);
        expect(new TextDecoder().decode(p2)).toBe('msg-2');
        // Bob receives msg 0 (from skipped keys)
        const { plaintext: p0, nextState: s2 } = DoubleRatchet.decrypt(s1, msgs[0].ratchetPublicKey, msgs[0].previousChainLength, msgs[0].messageNumber, msgs[0].nonce, msgs[0].ciphertext);
        expect(new TextDecoder().decode(p0)).toBe('msg-0');
        // Bob receives msg 1 (from skipped keys)
        const { plaintext: p1 } = DoubleRatchet.decrypt(s2, msgs[1].ratchetPublicKey, msgs[1].previousChainLength, msgs[1].messageNumber, msgs[1].nonce, msgs[1].ciphertext);
        expect(new TextDecoder().decode(p1)).toBe('msg-1');
    });
});
//# sourceMappingURL=double-ratchet.test.js.map