...
import { SenderKeys } from '../protocol/sender-keys.js';
describe('SenderKeys', () => {
    ...
    it('chain advances correctly across multiple messages', () => {
        ...
        for (let i = 0; i < 5; i++) {
            const { plaintext, nextRecord } = SenderKeys.decrypt(currentRecord, messages[i].chainIndex, messages[i].nonce, messages[i].ciphertext);
    ...
    it('handles out-of-order decryption', () => {
        ...
        const { plaintext: p2, nextRecord: r1 } = SenderKeys.decrypt(record, messages[2].chainIndex, messages[2].nonce, messages[2].ciphertext);
        ...
        const { plaintext: p0, nextRecord: r2 } = SenderKeys.decrypt(r1, messages[0].chainIndex, messages[0].nonce, messages[0].ciphertext);
        ...
        const { plaintext: p1, nextRecord: r3 } = SenderKeys.decrypt(r2, messages[1].chainIndex, messages[1].nonce, messages[1].ciphertext);
    ...
    it('throws on too many skipped messages (max 256)', () => {
        ...
        const { ciphertext, nonce, chainIndex } = SenderKeys.encrypt(advancedState, new TextEncoder().encode('too far'));
        expect(() => SenderKeys.decrypt(record, chainIndex, nonce, ciphertext)).toThrow(/too many skipped/i);
    });
